...
class AppointmentRepository {
  ...
  /**
   * Returns rich appointment rows for one staff member, joining client name,
   * service name, category name, duration, and price.
   * @param {number|string} staff_id
   * @returns {Promise<Array<Object>>}
   */
  static async findStaffAppointmentsRich(staff_id) {
    const q = `
      SELECT
        a.appointment_id,
        a.appointment_start_at,
        ...
        client.user_email    AS client_email,
      ...
      LEFT JOIN users client ON client.user_id = a.client_id
      ...
      LEFT JOIN (
        SELECT DISTINCT ON (staff_id)
          staff_id, service_id,
          staff_duration_min, staff_price_cents
        FROM staff_services
        ORDER BY staff_id, service_id
      ) ss_fb ON ss_fb.staff_id = a.staff_id AND ai.appointment_id IS NULL
   ...
   * @returns {Promise<Array<Object>>}
   */
  static async findAllAppointmentsRich() {
    const q = `
      SELECT
        a.appointment_id,
        a.appointment_start_at,
      ...
      LEFT JOIN users client ON client.user_id = a.client_id
      LEFT JOIN users staff  ON staff.user_id  = a.staff_id
